...
int main() {
  ...
  while(1) {
    ...
    if(isOpen) {
      ...
      } else printf("Error: File system image must be opened first.\n\n");
      ...
      if(isOpen) {
        ...
        for (; i < 16; i++) {
          ...
          if (dir[i].DIR_attr != 1 && dir[i].DIR_attr != 16 && dir[i].DIR_attr != 32) continue;
          ...
          // debug
          //printf("\ncheck:\n%s  len: %d\n%s  len: %d\n", name, strlen(name), str, strlen(name));
      ...
      if(isOpen) {
        ...
        for (; i < 16; i++) {
          ...
          if (dir[i].DIR_attr != 1 && dir[i].DIR_attr != 32) continue; 
